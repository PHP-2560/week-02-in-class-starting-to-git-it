...
```{r}
help(read.table) # Look up the arguments to read.table
rain.df <- read.table("rnf6080.dat") # Load the .dat file and call it rain.df
```
...
```{r}
dim(rain.df) # get the number of rows and columns
```

3. What are the names of the columns of rain.df?

```{r}
names(rain.df) # Get column names
# The column names are V1, V2, V3, etc...not very informative!
```

4. What is the value of row 5, column 7 of rain.df?

```{r}
rain.df[5,7] # get value of row 5, column 7
```
...
```{r}
rain.df[2,] # get values in the entire second row
```
...
head(rain.df)
tail(rain.df)
# The 24 columns represent hourly rainfall for the 24 hours in each day
...
names(rain.df)
hist(rain.df$daily)
# There are a bunch of negative values of daily rainfall. I'm guessing that missing values are coded with a negative 
# number, and we're summing them when we create the daily variable. We should probably replace these values with NA!
...
```{r}
# Let's code anything lower than zero in the "daily" column as missing:
length(which(rain.df$daily < 0))

rain.df.fixed <- rain.df %>% mutate_all(funs(replace(., .<0, NA)))
